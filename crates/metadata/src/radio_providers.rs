...
pub fn process_radiosphere_meta(agent: &ureq::Agent, final_url: &str, radio_meta: &mut RadioMeta) {
    ...
    if let Some(url) = track_url
        && let Ok(api_resp) = agent.get(&url).call()
        && let Ok(body) = api_resp.into_body().read_to_string()
        && let Some(song) = parse_song_metadata(&body)
    {
        radio_meta.name = Some(format!("{} - {}", song.artist.unwrap_or_default(), song.title.unwrap_or_default()));
    }
