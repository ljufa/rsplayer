...
pub enum BiquadParameters {
    ...
    LinkwitzTransform { freq_act: f32, q_act: f32, freq_target: f32, q_target: f32 },
}
...
impl BiquadCoefficients {
    ...
    pub fn from_config(fs: usize, params: BiquadParameters) -> Self {
        ...
        match params {
            ...
            BiquadParameters::LinkwitzTransform { freq_act, q_act, freq_target, q_target } => {
                ...
                Self::normalize(
                    1.0, 2.0 * (c0i - gn2) / cci, (c0i - gn * c1i + gn2) / cci,
                    (d0i + gn * d1i + gn2) / cci,
                    2.0 * (d0i - gn2) / cci,
                    (d0i - gn * d1i + gn2) / cci,
                )
...
impl Filter for Biquad {
    fn process_waveform(&mut self, buf: &mut [f32]) -> anyhow::Result<()> {
        ...
        if self.s1.is_subnormal() { self.s1 = 0.0; }
        if self.s2.is_subnormal() { self.s2 = 0.0; }
        Ok(())
    }
}
