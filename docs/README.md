...
RSPlayer is a headless music server primarily for Linux, with experimental macOS binaries, designed for audiophile-grade local playback on devices like the Raspberry Pi.
...
- **Software Volume**: Platform-independent software gain volume backend
- **Browser Playback**: Stream to your web browser
- **Music Visualizer**: 12 animated visualizer styles
...
- **macOS Targets (experimental)**: `aarch64-apple-darwin`, `x86_64-apple-darwin`
...
- **Local Browser Playback**: The browser's native audio engine handles playback directly. DSP, loudness normalization, resampling, visualization, and DSD are all unavailable in this mode.
- **Non-Linux builds**: Network mount management and Linux power controls are unavailable.
