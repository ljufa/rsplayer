...
*   **rsplayer**: A lightweight, efficient, and free solution ideal for **DIY enthusiasts**. Setup is as simple as running a single install script — no OS image, no separate client required. It excels in direct hardware integration (OLEDs, VU meters, IR) and offers low-level control via **HA and USB command channels**. It features a robust **Rust-based DSP engine** with parametric EQ, various audio filters, and **real-time synchronized lyrics** via LRCLIB.
